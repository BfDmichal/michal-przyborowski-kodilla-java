...
public class BoardTestSuite {
    public Board prepareTestData() {
        ...
        //tasks
        Task task1 = new Task("Microservice for taking temperature", "White and test the microservice taking\n" + "the temperatur from external service", user1, user2, LocalDate.now().minusDays(20), LocalDate.now().plusDays(30));
        Task task2 = new Task("HQLs for analiysis", "Prepare some HQL queries for analysis", user1, user2, LocalDate.now().minusDays(20), LocalDate.now().minusDays(5));
    ...
    public void testAddTaskListFindOutdatedTasks() {
        ...
        List<Task> list = project.getTaskLists().stream()
                .filter(n -> n.getName().equals("In progress") || n.getName().equals("To do"))
    ...
    public void testAddTaskListFindLongTask() {
        //Given
        Board project = prepareTestData();
        TaskList list1 = new TaskList("In progress");
        //When
        long numberOfTasks = project.getTaskLists().stream()
                .filter(list1::equals)
                //.filter(taskList -> taskList.equals(list1))
    ...
    public void testAddTaskListAverageWorkingOnTask() {
        ...
        double average = project.getTaskLists().stream()
                ...
                .flatMap(taskList -> taskList.getTasks().stream())
                .mapToInt(value -> Period.between(value.getCreated(), LocalDate.now()).getDays())
                .average().getAsDouble();
        //Then
